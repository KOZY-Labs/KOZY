...
export async function signUpWithEmail({ email, password, profile = {} }) {
  ...
  // Public display name; the legal name above is for identity verification only.
  const displayName = (profile.displayName ?? '').trim() || firstName;
...
// Cap the cold-start profile read: Firestore's getDoc waits indefinitely for a
// connection, and a device that can't reach Google (seen in the field: restored
// session + broken Play/Google connectivity) would otherwise never resolve auth —
// the splash overlay then blocks every touch and the app looks frozen. On timeout
// we release with profile = null; AuthContext's live subscription fills it in
...
export function subscribeToAuth(callback) {
  return onAuthStateChanged(auth, async (user) => {
    ...
    let profile = null;
    try {
      profile = await withTimeout(getUserDoc(user.uid), PROFILE_FETCH_TIMEOUT_MS);

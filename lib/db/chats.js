...
export function subscribeToChats(uid, callback) {
  ...
  return onSnapshot(
    q,
    (snap) => callback(snap.docs.map(mapSnap)),
    (e) => {
      // Expected during auth transitions (logout, account delete/switch): the token
      // has already changed while React hasn't torn this listener down yet, so the
      // server rejects a query for the OLD uid. Stale → silent; a denial for the
      // current user is a real rules problem and still warns.
      if (e?.code === 'permission-denied' && auth.currentUser?.uid !== uid) return;
      console.warn('[firestore] chats list listener:', e?.code ?? e?.message ?? e);
    }
  );
}
...
export function subscribeToChat(chatId, callback) {
  return onSnapshot(
    ...
    (e) => {
      console.warn('[firestore] chat doc listener:', e?.code ?? e?.message ?? e);
    }
  );
}
...
// Keep in sync with functions/src/notifications.js (push body uses the same text).
const previewFor = (type, text, count) =>
  type === 'image' && count > 1 ? `📷 ${count} Photos` : (MEDIA_PREVIEW[type] ?? text);
...
export async function sendMessage(
  chatId,
  { senderId, text = '', type = 'text', status = 'sent', mediaUrl = null, mediaUrls = null, otherIds = [] }
) {
  // Multi-photo sends carry `mediaUrls` (2+); single media keeps the legacy
  // `mediaUrl` so older messages and readers stay compatible.
  const urls = Array.isArray(mediaUrls) && mediaUrls.length > 1 ? mediaUrls : null;
  const single = mediaUrl ?? (mediaUrls?.length === 1 ? mediaUrls[0] : null);
  const ref = await addDoc(messagesRef(chatId), {
    senderId,
    text,
    type,
    status,
    ...(urls ? { mediaUrls: urls } : single ? { mediaUrl: single } : {}),
    createdAt: serverTimestamp(),
  });
  await updateDoc(chatRef(chatId), {
    lastMessage: previewFor(type, text, urls?.length ?? 1),
    lastMessageAt: serverTimestamp(),
    updatedAt: serverTimestamp(),
...
export function subscribeToMessages(chatId, callback) {
  ...
  return onSnapshot(
    ...
    (snap) => callback(snap.docs.map(mapSnap)),
    (e) => {
      // Terminal listener error (e.g. rules): keep the last value, name the listener.
      console.warn('[firestore] messages listener:', e?.code ?? e?.message ?? e);
    }
  );
}

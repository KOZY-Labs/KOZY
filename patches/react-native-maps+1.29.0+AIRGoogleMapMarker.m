...
// Bounded retry count for the deferred custom-marker-children icon fit (see
// -airMapsFitIconViewWithAttemptsRemaining:). One "attempt" is one main-queue
// run-loop turn, which is enough for Fabric to commit the children's layout
// metrics in the mutation(s) that follow the initial mount.
...
- (void) didInsertInMap:(AIRGoogleMap *) map
{
    ...
    // Fabric/New Architecture fix: the iconView may have been assigned to the
    // GMSMarker while it was off-map (e.g. on zIndex-triggered remove+insert).
    // With tracksViewChanges == NO, GMS only rasterizes the icon view when it is
    // (re)assigned while attached to a map, so force a redraw now that we're on-map.
...
- (void)iconViewInsertSubview:(UIView*)subview atIndex:(NSInteger)atIndex {
    ...
    // Fabric/New Architecture fix: under Fabric this view is detached from the
    // UIKit hierarchy, so layoutSubviews/didUpdateReactSubviews never fire and
    // _iconView keeps a CGRectZero frame, making custom marker children invisible.
    // Children receive their layout metrics in a later mutation of the same (or a
    // subsequent) mount transaction, so retry a few times on the next run loop
    // turns until a non-zero child size shows up, then size + rasterize the icon.
...
- (void)airMapsFitIconViewWithAttemptsRemaining:(NSInteger)attemptsRemaining {
    ...
    for (UIView *v in [_iconView subviews]) {
        if (v.frame.size.width > 0 && v.frame.size.height > 0) {
    ...
    if (_realMarker.iconView && _realMarker.map) {
        [self redraw];
    }
}
